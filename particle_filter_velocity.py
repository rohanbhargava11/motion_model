...
class robot(object):
    ...
    def move_velocity(self, turn_velocity, forward_velocity,independent_noise_trans,independent_noise_turn):
            time=1.0
            ...
            var_trans_independent=independent_noise_trans
            var_rot_independent=independent_noise_turn                
            forward_velocity=random.gauss(forward_velocity,var_trans_independent)
            #print forward_velocity
            turn_velocity=random.gauss(turn_velocity,var_rot_independent)
            #print turn_velocity
            orientation=self.orientation+turn_velocity*time
            orientation %= 2 * pi                
...
def error_calculation(traj,trans_readings,rotation_readings):
    ...
    for i in range(len(traj)):
        for j in range(len(traj[i])-1):
            error_rotation[i][j]=np.mod((((traj[i][j+1][2]-traj[i][j][2])/time)-rotation_readings[j]*time),2*np.pi)#traj[i][j+1][2]-traj[i][j][2]# 
            
            error_trans[i][j]=(-((min(abs(traj[i][j+1][0]-traj[i][j][0]),100-abs(traj[i][j+1][0]-traj[i][j][0])))/time)/((traj[i][j+1][2]-traj[i][j][2])/time))*sin(traj[i][j][2])+(((min(abs(traj[i][j+1][0]-traj[i][j][0]),100-abs(traj[i][j+1][0]-traj[i][j][0])))/time)/((traj[i][j+1][2]-traj[i][j][2])/time))*sin(traj[i][j][2]+error_rotation[i][j]+rotation_readings[j]*time)+(((min(abs(traj[i][j+1][0]-traj[i][j][0]),100-abs(traj[i][j+1][0]-traj[i][j][0])))/time)/((traj[i][j+1][2]-traj[i][j][2])/time))*cos(traj[i][j][2])-(((min(abs(traj[i][j+1][0]-traj[i][j][0]),100-abs(traj[i][j+1][0]-traj[i][j][0])))/time)/((traj[i][j+1][2]-traj[i][j][2])/time))*cos(traj[i][j][2]+error_rotation[i][j]+rotation_readings[j]*time)
            #print 'Actual movement',error_rotation,error_trans
            error_trans[i][j]=abs(error_trans[i][j])-trans_readings[j]*time
...
def f(x,*args):
    ...
    #print c
    #print c[:][0]
    #print np.log((a[0:counter]**2)*u + (b[0:counter]**2)*v + z)
    #print sum(np.log(2*np.pi)+np.log(z)+((c[:,0:counter]**2)/z))*(-0.5)
    return sum(np.log(2*np.pi)+np.log(z)+((c[:,0:counter]**2)/z))*(-0.5)
    #temp=temp*(-1*0.5)    
    #return temp
...
distance_reported=np.zeros((timesteps))

rotation_reported=np.zeros((timesteps))
...
p_history=np.ndarray((500,timesteps),dtype=np.object)
p_decay_history=np.ndarray((500,timesteps),dtype=np.object)
#show()
...
move=2.0
rotate=0.1  #is this in radian or degrees???
for t in range(timesteps):
    ...
    if t<5:
        myrobot = myrobot.move_velocity(rotate,move,0.05,0.05) #turn,forward
    else:
        print 'I am on a different terrain'
        myrobot=myrobot.move_velocity(rotate,move,0.5,0.05) # still have to deal with the 2 theing
    ...
    for i in range(N):
        p2.append(p[i].move_velocity(rotate,move,inde,inde_turn))
        p2_original.append(p_original[i].move_velocity(rotate,move,0.05,0.05))
        #p2_decay_original.append(p_decay_original[i].move_velocity(rotate,move,forw_decay,turn_decay,inde_decay,forw_turn_decay,turn_turn_decay,inde_turn_decay))
    ...
    if len(trajectory[0])>1:
        ...
        #print tran_error
        res1 = optimize.fminbound(f,0,10,args=args,full_output=True)
        res2 = optimize.fminbound(f,0,10,args=args1,full_output=True)
        #print 'rotation'
    ...
    #print 'particle',particle_location
    #print 'particle original',particle_location_original
