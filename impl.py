...
bearing_noise =1.0 # Noise parameter: should be included in sense function.
...
landmarks  = [[0.0, 200.0], [0.0, 0.0], [200.0, 0.0], [200.0, 200.0]] # position of 4 landmarks in (y, x) format.
world_size = 600.0 # world is NOT cyclic. Robot is allowed to travel "out of bounds"
...
class robot:
    ...
    def __init__(self, length = 20.0, init = 0):
        if(init == 0):
          ...
          # if I don't use random and tell to go move at fixed places then I can compare both the methods
          #self.x=world_size
          #self.y=world_size
    ...
    #
    def set_noise(self, new_b_noise, new_s_noise, new_d_noise,new_t_noise,new_f_noise):
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            error_bearing = abs(measurements[i] - predicted_measurements[i]) # but this would just say that measurements are same and would give no error 
            error_bearing = (error_bearing + pi) % (2.0 * pi) - pi # truncate
            # I don't see the point of limitng the sensor
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        result.set_noise(self.bearing_noise, self.steering_noise, self.distance_noise,self.turn_noise,self.forward_noise)
    ...
    def move(self, turn, forward):
       ...
       res.set_noise(self.bearing_noise,self.steering_noise,self.distance_noise,self.turn_noise, self.forward_noise)
       return res    
    '''
    # --------
    # sense: 
    ...
    def sense(self, noise_flag=1):
        ...
        for landmark in landmarks:
            ...
            # I can restrict the distance that would mean that it cannot see all the landmarks 
...
def generate_ground_truth(motions):
    ...
    myrobot.set_noise(bearing_noise, steering_noise, distance_noise,turn_noise,forward_noise)
